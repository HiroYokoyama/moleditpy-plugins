...
def _declared(const):
    """Read a PLUGIN_* constant from the package __init__ without importing it."""
    import re as _re

    src = (MO_PKG_DIR / "__init__.py").read_text(encoding="utf-8")
    m = _re.search(rf"""{const}\s*=\s*['"](.+?)['"]""", src)
...
class TestMOCubeWriter:
    ...
    def _write_as_package(self, tmp_path):
        """Write with the relative import resolvable, as in the real app.

        analyzer.py reads PLUGIN_NAME/PLUGIN_VERSION off the package at call
        time (__init__ imports analyzer before defining them, so a module-level
        import would be circular). Loaded flat by load_plugin, that relative
        import fails; register a stand-in package so the real branch runs.
        """
        pkg_name = "_gmo_pkg_for_stamp"
        pkg = types.ModuleType(pkg_name)
        pkg.__path__ = [str(MO_PKG_DIR)]
        pkg.PLUGIN_NAME = _declared("PLUGIN_NAME")
        pkg.PLUGIN_VERSION = _declared("PLUGIN_VERSION")
        saved_pkg = sys.modules.get(pkg_name)
        saved_attr = _mo_real.__package__
        sys.modules[pkg_name] = pkg
        _mo_real.__package__ = pkg_name
        try:
            return self._write(tmp_path, 6)
        finally:
            _mo_real.__package__ = saved_attr
            if saved_pkg is None:
                sys.modules.pop(pkg_name, None)
            else:
                sys.modules[pkg_name] = saved_pkg

    def test_second_line_stamps_name_and_version(self, tmp_path):
        """Cube provenance must be readable from the file itself.

        The f/g normalization fix changed the values in every cube built from
        a basis carrying those shells, so "which version wrote this" cannot be
        answered by inspecting the numbers.
        """
        lines = self._write_as_package(tmp_path)
        assert "MoleditPy" in lines[1]
        assert _declared("PLUGIN_NAME") in lines[1]
        assert f"v{_declared('PLUGIN_VERSION')}" in lines[1]
        assert "unknown" not in lines[1]

    def test_stamp_does_not_shift_the_grid_header(self, tmp_path):
        """Cube format is positional: the atom count stays on line 3."""
        lines = self._write_as_package(tmp_path)
        assert int(lines[2].split()[0]) == 1

    def test_flat_load_falls_back_without_crashing(self, tmp_path):
        """Loaded outside the package the stamp degrades, it does not raise."""
        lines = self._write(tmp_path, 6)
        assert lines[1].startswith("Generated by MoleditPy")
