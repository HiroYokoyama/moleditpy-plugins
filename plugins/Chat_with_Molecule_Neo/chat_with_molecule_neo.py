...
class NameResolverWorker(QRunnable):
    ...
    def run(self):
        try:
            ...
            logging.warning("[chat_with_molecule_neo.py:528] silenced: %s", _e)
...
class ChatMoleculeWindow(QDialog):
    ...
    def init_ui(self):
        ...
        if icon_path and os.path.exists(icon_path):
            try:
                ...
                logging.warning("[chat_with_molecule_neo.py:756] silenced: %s", _e)
        ...
        # Link to get key
        lbl_link = QLabel(
            '<a href="https://aistudio.google.com/app/api-keys">Get API Key</a>'
        )
    ...
    def initialize_session(self):
        ...
        if not HAS_GENAI:
            self.append_message(
                "System",
                "Error: 'google-generativeai' library is not installed.",
                "red",
            )
        ...
        if not api_key:
            self.append_message(
                "System", "Please enter your Google API Key above.", "orange"
            )
            QMessageBox.warning(
                self,
                "Welcome",
                "Please enter your Google Gemini API Key in the settings above to start chatting.",
            )
    ...
    def on_init_finished(self, available_models, error_msg):
        ...
        if not available_models:
            self.append_message(
                "System", "No models found that support 'generateContent'.", "red"
            )
            return
    ...
    def get_current_molecule_smiles(self):
        ...
        try:
            if hasattr(self.main_window.state_manager, "data") and hasattr(
                self.main_window.state_manager.data, "to_rdkit_mol"
            ):
                if (
                    hasattr(self.main_window.state_manager.data, "atoms")
                    and self.main_window.state_manager.data.atoms
                ):
                    ...
                    try:
                        ...
                        sorted_ids = sorted(
                            self.main_window.state_manager.data.atoms.keys()
                        )
                        ...
                        # 2. Add Bonds
                        if (
                            hasattr(self.main_window.state_manager.data, "bonds")
                            and self.main_window.state_manager.data.bonds
                        ):
                            sorted_bids = sorted(
                                self.main_window.state_manager.data.bonds.keys(),
                                key=str,
                            )
                            for bid in sorted_bids:
                                bond_data = self.main_window.state_manager.data.bonds[
                                    bid
                                ]
                                ...
                                if id1 in id_map and id2 in id_map:
                                    bond_data = (
                                        self.main_window.state_manager.data.bonds[bid]
                                    )
        ...
        if mol is None:
            try:
                ...
                logging.warning("[chat_with_molecule_neo.py:1496] silenced: %s", _e)

        if mol is None:
            if (
                hasattr(self.main_window.state_manager, "data")
                and hasattr(self.main_window.state_manager.data, "atoms")
                and not self.main_window.state_manager.data.atoms
            ):
    ...
    def load_smiles_undo_safe(self, smiles_string):
        ...
        try:
            ...
            try:
                ...
                logging.warning("[chat_with_molecule_neo.py:1657] silenced: %s", _e)
    ...
    def on_name_resolved(self, inchikey, name):
        ...
        if self.last_inchikey == inchikey:
            ...
            self.check_molecule_change()
        self.pending_calc_results = (
            ""  # Clear any previous calculation results when a new name is resolved
        )
    ...
    def execute_apply_transformation(self, params):
        ...
        try:
            ...
            try:
                ...
                logging.warning("[chat_with_molecule_neo.py:2196] silenced: %s", _e)
            ...
            self.append_message(
                "System",
                f"Transformation Failed: {e}\n(Tip: The SMARTS pattern might be invalid. Ask Gemini for a more precise one.)",
                "red",
            )
            return str(e)
    ...
    def execute_calculate_descriptors(self, params):
        ...
        try:
            ...
            self.append_message(
                "System",
                f"Calculation Error: {e}\n(Tip: Ask Gemini to ensure the molecule is valid.)",
                "red",
            )
            return str(e)
    ...
    def execute_load_molecule(self, params):
        ...
        try:
            ...
            self.append_message(
                "System",
                f"Load Error: {e}\n(Tip: The SMILES string might be invalid. Ask Gemini for a more precise one.)",
                "red",
            )
            return str(e)
    ...
    def _get_molecule_for_export(self):
        ...
        if not mol_to_export:
            ...
            try:
                ...
                logging.warning("[chat_with_molecule_neo.py:2700] silenced: %s", _e)
            self.append_message(
                "System",
    ...
    def _build_context_msg(self, smiles, name, lazy=False):
        ...
        try:
            ...
            logging.warning("[chat_with_molecule_neo.py:2893] silenced: %s", _e)
        ...
        if current_smiles:
            ...
            if not mol_name and self.last_inchikey and PubChemResolver:
                ...
                try:
                    ...
                    logging.warning("[chat_with_molecule_neo.py:2963] silenced: %s", _e)
        ...
        try:
            ...
            if prev_conversion_id != -1 and curr_conversion_id == prev_conversion_id:
                ...
                try:
                    ...
                    logging.warning("[chat_with_molecule_neo.py:3251] silenced: %s", _e)
                self.append_message(
    ...
    def on_final_response(self, response):
        ...
        self.chat_history_log.append(
            {"sender": "Gemini", "text": self.stream_accumulated_text}
        )
