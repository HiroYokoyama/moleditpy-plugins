...
class VectorViewerPlugin(QWidget):
    ...
    def update_color_btn_style(self):
        ...
        self.color_btn.setStyleSheet(
            f"background-color: {name}; color: {text_col}; font-weight: bold;"
        )
    ...
    def update_visualization(self):
        ...
        arrow = pv.Arrow(
            start=start,
            direction=scaled_vec,
            scale=mag,
            tip_length=0.2,
            tip_radius=0.1,
            shaft_radius=0.04,
            shaft_resolution=res,
            tip_resolution=res,
        )

        # Add to plotter
        self.vis_actor = plotter.add_mesh(
            arrow, color=color, opacity=opacity, name="vector_viewer_arrow"
        )
        plotter.render()
