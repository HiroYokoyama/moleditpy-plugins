    ...
    def populate_list(self):
        ...
        if mode == 0:
            ...
            for r in range(self.list_widget.count()):
                ...
                if (
                    "HOMO" in item.text() and "-" not in item.text().split("HOMO")[-1]
                ):  # Exact HOMO match or check UserRole
    ...
    def on_finished(self, success, result):
        if success:
            ...
            QMessageBox.warning(
                self,
                "Error",
                f"Calculation failed for MO {self.current_gen_mo_idx}: {result}",
            )
    ...
    def visualize(self, cube_path):
        ...
        if vis.load_file(cube_path):
            ...
            self.context.show_status_message(
                f"Visualizing {os.path.basename(cube_path)} (Iso={val}, Opacity={op_val}, Smooth={is_smooth})"
            )
        else:
