...
PLUGIN_DESCRIPTION = (
    "Take high-resolution screenshots of the 3D view with custom resolution."
)
PLUGIN_CONTEXT = None
...
def initialize(context):
    ...
    context.add_export_action(
        "High Resolution Screenshot...", lambda: take_screenshot(context)
    )
...
def take_screenshot(context):
    ...
    layout.addWidget(
        QLabel(
            "Note: Resolution settings apply to raster images only (PNG, JPG, BMP, TIFF)"
        )
    )
    ...
    def pick_bg_color():
        c = QColorDialog.getColor(
            QColor(selected_bg_color[0]), dialog, "Select Background Color"
        )
        if c.isValid():
    ...
    if dialog.exec() == QDialog.DialogCode.Accepted:
        ...
        # File Dialog
        filename, filter_selected = QFileDialog.getSaveFileName(
            mw,
            "Save Screenshot",
            "",
            "PNG Images (*.png);;JPEG Images (*.jpg *.jpeg);;BMP Images (*.bmp);;TIFF Images (*.tif *.tiff);;SVG Images (*.svg);;All Files (*)",
        )
        if filename:
            try:
                ...
                if rb_custom.isChecked():
                    ...
                    is_vector = filename.lower().endswith(
                        (".svg", ".pdf", ".eps", ".ps")
                    )
                    if is_vector:
                        target_bg = "white"
