...
class OrcaKeywordBuilderDialog(QDialog):
    ...
    def setup_method_tab(self):
        ...
        self.aux_basis.addItems(
            ["Auto (Def2/J, etc)", "None", "Def2/J", "Def2/JK", "AutoAux", "NoAux"]
        )
        ...
        if "GGA/Hybrid" in mtype:
            self.method_name.addItems(
                [
                    ...
                    "O3LYP",
                    "B3PW91",
                    "BHandHLYP",  # BH&HLYPをBHandHLYPに修正
                ]
            )
            ...
            self.method_name.addItems(
                ["XT", "GFN1-xTB", "GFN2-xTB", "PM3", "AM1", "ZINDO/S", "PM6", "MNDO"]
            )
        else:
    ...
    def setup_props_tab(self):
        ...
        self.grid_combo.addItems(
            ["Default", "DefGrid2", "DefGrid3", "Grid4", "Grid5", "Grid6", "NoGrid"]
        )
    ...
    def update_preview(self):
        ...
        if self.opt_group.isVisible():
            if self.opt_tight.isChecked():
                route_parts.append("TightOpt")
            if self.opt_loose.isChecked():
                route_parts.append("LooseOpt")
...
class OrcaSetupDialogNeo(QDialog):
    ...
    def setup_ui(self):
        ...
        self.coord_format_combo.addItems(
            [
                "Cartesian (XYZ)",
                "Internal (Z-Matrix / * int)",
                "Internal (Z-Matrix / * gzmt)",
            ]
        )
        ...
        self.adv_tabs.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
        )
        self.adv_tabs.setFixedHeight(150)

        self.adv_edit = QTextEdit()
        self.adv_edit.setPlaceholderText(
            "Pre-Coordinate Blocks\\nExample:\\n%scf\\n MaxIter 100\\nend"
        )
    ...
    def get_coords_lines(self):
        ...
        try:
            ...
            for i in range(self.mol.GetNumAtoms()):
                ...
                lines.append(
                    f"  {atom.GetSymbol(): <4} {pos.x: >12.6f} {pos.y: >12.6f} {pos.z: >12.6f}"
                )
        except Exception as e:
            return [f"# Error: {e}"]
        return lines
    ...
    def generate_input_content(self):
        ...
        if is_cartesian:
            ...
            if any("Error" in line for line in zmat_lines):
                ...
                content.append(
                    f"* xyz {self.charge_spin.value()} {self.mult_spin.value()}"
                )
                content.extend(self.get_coords_lines())
                content.append("*")
            else:
                content.append(
                    f"{header} {self.charge_spin.value()} {self.mult_spin.value()}"
                )
