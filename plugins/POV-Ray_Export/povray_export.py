...
def initialize(context):
    ...
    context.add_export_action(
        "Export to POV-Ray Scene...", lambda: export_to_povray(context)
    )
...
def export_to_povray(context):
    ...
    try:
        ...
        PLUGIN_CONTEXT.show_status_message(
            f"POV-Ray scene exported to {file_path}", 3000
        )
        ...
        QMessageBox.critical(
            mw, "Export Failed", f"Failed to export POV-Ray scene:\n{str(e)}"
        )
...
def generate_povray_scene(mol, mw):
    ...
    lines.append("//")
    lines.append(
        "// NOTE: To get high-resolution output, select your desired resolution"
    )
    ...
    lines.append(
        f"background {{ color rgb <{bg_rgb[0]:.3f}, {bg_rgb[1]:.3f}, {bg_rgb[2]:.3f}> }}"
    )
    lines.append("")

    lines.append(
        "// Sky sphere for reflections - using bozo for smooth, edge-free background"
    )
    ...
    lines.append(
        f"      [0.0 color rgb <{bg_rgb[0] * 0.95:.3f}, {bg_rgb[1] * 0.95:.3f}, {bg_rgb[2] * 0.95:.3f}>]"
    )
    lines.append(
        f"      [0.5 color rgb <{bg_rgb[0]:.3f}, {bg_rgb[1]:.3f}, {bg_rgb[2]:.3f}>]"
    )
    lines.append(
        f"      [1.0 color rgb <{min(1.0, bg_rgb[0] * 1.05):.3f}, {min(1.0, bg_rgb[1] * 1.05):.3f}, {min(1.0, bg_rgb[2] * 1.05):.3f}>]"
    )
    lines.append("    }")
    ...
    lines.append(
        f"  <{loc[0] + light_dist}, {loc[1] + light_dist}, {loc[2] + light_dist}>"
    )
    ...
    lines.append(
        f"  roughness {roughness_val * 2 if roughness_val * 2 < 1.0 else 1.0:.3f}"
    )
    lines.append("}")
    lines.append("*/")
    lines.append("")
    ...
    if current_style != "wireframe":
        ...
        if current_style == "stick":
                    ...
                    if bt in [Chem.BondType.DOUBLE, Chem.BondType.TRIPLE]:
                        ...
                        for off in offsets:
                            atoms_to_draw.append(
                                (pos_i + off, sphere_rad, color, atom.GetSymbol(), idx)
                            )
            ...
            for atom in mol.GetAtoms():
                ...
                color_key = f"atom_{idx}"
                color = (
                    tuple(c / 255.0 for c in color_map[color_key])
                    if color_key in color_map
                    else cpk_colors.get(symbol, (0.8, 0.8, 0.8))
                )
                radius = (
                    float(actual_radii[idx])
        ...
        for pos, radius, color, symbol, idx in atoms_to_draw:
            ...
            lines.append(
                f"    pigment {{ color rgb <{color[0]:.3f}, {color[1]:.3f}, {color[2]:.3f}> }}"
            )
    ...
    if current_style != "cpk":
        ...
        for bond in mol.GetBonds():
            ...
            def add_pov_segment(p1, p2, radius, rgb_255, comment):
                ...
                lines.append("  texture {")
                lines.append(
                    f"    pigment {{ color rgb <{rgb[0]:.3f}, {rgb[1]:.3f}, {rgb[2]:.3f}> }}"
                )
            ...
            if (
                ...
                if f"bond_{bond_counter}" in color_map:
                    ...
                    add_pov_segment(
                        ...
                        pos2,
                        cyl_radius,
                        color_map[f"bond_{bond_counter}"],
                        f"Bond {bond_counter}",
                    )
                    ...
                    add_pov_segment(
                        ...
                        mid,
                        cyl_radius,
                        color_map[f"bond_{bond_counter}_start"],
                        f"Bond {bond_counter} start",
                    )
                    add_pov_segment(
                        mid,
                        pos2,
                        cyl_radius,
                        color_map[f"bond_{bond_counter}_end"],
                        f"Bond {bond_counter} end",
                    )
                else:
                    # Fallback to gray
                    add_pov_segment(
                ...
                if bond_type == Chem.rdchem.BondType.DOUBLE:
                    ...
                    p1_start, p1_end = (
                        pos1 + off_dir * (s_double / 2),
                        pos2 + off_dir * (s_double / 2),
                    )
                    p2_start, p2_end = (
                        pos1 - off_dir * (s_double / 2),
                        pos2 - off_dir * (s_double / 2),
                    ...
                    if f"bond_{bond_counter}_1" in color_map:
                        add_pov_segment(
                            ...
                            p1_end,
                            r,
                            color_map[f"bond_{bond_counter}_1"],
                            f"Bond {bond_counter} segment 1",
                        )
                    else:
                        ...
                        add_pov_segment(
                            ...
                            f"Bond {bond_counter} seg 1 start",
                        )
                        add_pov_segment(
                            mid,
                            p1_end,
                    ...
                    if f"bond_{bond_counter}_2" in color_map:
                        add_pov_segment(
                            ...
                            p2_end,
                            r,
                            color_map[f"bond_{bond_counter}_2"],
                            f"Bond {bond_counter} segment 2",
                        )
                    else:
                        ...
                        add_pov_segment(
                            ...
                            f"Bond {bond_counter} seg 2 start",
                        )
                        add_pov_segment(
                            mid,
                            p2_end,
                    ...
                    if f"bond_{bond_counter}_1" in color_map:
                        ...
                        add_pov_segment(
                            pos1, mid, r, ccol1, f"Bond {bond_counter} center start"
                        )
                        add_pov_segment(
                            mid, pos2, r, ccol2, f"Bond {bond_counter} center end"
                        )
                    ...
                    for i, sign in enumerate([1, -1], 2):
                        ...
                        if f"bond_{bond_counter}{suffix}" in color_map:
                            add_pov_segment(
                                ...
                                p_end,
                                r,
                                color_map[f"bond_{bond_counter}{suffix}"],
                                f"Bond {bond_counter} side {i - 1}",
                            )
                        else:
                            mid = (p_start + p_end) / 2
                            add_pov_segment(
                                ...
                                f"Bond {bond_counter} side {i - 1} start",
                            )
                            add_pov_segment(
                                mid,
                                p_end,
                                ...
                                f"Bond {bond_counter} side {i - 1} end",
                            )
    ...
    lines.append("  texture {")
    lines.append(
        f"    pigment {{ color rgb <{bg_rgb[0] * 0.8:.3f}, {bg_rgb[1] * 0.8:.3f}, {bg_rgb[2] * 0.8:.3f}> }}"
    )
    lines.append("    finish { ")
    ...
    lines.append(
        f"// End of scene - {mol.GetNumAtoms()} atoms, {mol.GetNumBonds()} bonds"
    )
