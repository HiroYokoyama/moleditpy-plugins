...
class PluginInstallerWindow(QDialog):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(
                        "Plugin Installer: failed to detect version gracefully: %s", e
                    )
                    return "0.0.0"
    ...
    def check_updates(self):
        ...
        self.lbl_latest_version.setText(
            f"<b>Latest ({pkg_name} on PyPI):</b> Checking..."
        )
        ...
        self.lbl_latest_version.setText(
            f"<b>Latest ({pkg_name} on PyPI):</b> <span style='color:{color}'>{status_text}</span>"
        )
    ...
    def populate_table(self, silent=False):
        ...
        for name in all_names:
            ...
            if silent and not (
                is_installed and can_update and status == "Update Available"
            ):
            ...
            if not is_compatible and supported_ver_str:
                status_item.setToolTip(
                    f"Requires MoleditPy {supported_ver_str} (Current: {app_ver})"
                )
                ...
                if label:
                    ...
                    if not is_installed and not is_compatible:
                        btn_action.setToolTip(
                            f"Warning: Incompatible with MoleditPy (Requires: {supported_ver_str})"
                        )
    ...
    def show_plugin_details(self, row, col):
        ...
        supported_version = "Unknown"
        if remote_info:
            supported_version = remote_info.get(
                "supported_moleditpy_version", "Unknown"
            )
    ...
    def on_update_clicked(self):
        ...
        if dependencies:
            ...
            if missing_deps:
                ...
                description = (
                    remote_info.get("description", "No description available.")
                    if remote_info
                    else ""
                )
                author = (
                        ...
                        try:
                            ...
                            if target_file and os.path.exists(target_file):
                                ...
                                if (
                                    ...
                                    try:
                                        logging.info(
                                            "Plugin Installer: overwriting existing plugin file: %s",
                                            target_file,
                                        )
                                        ...
                                        logging.warning(
                                            "Plugin Installer: manual overwrite failed: %s — falling back to manager",
                                            e,
                                        )
                                ...
                                # Check: Folder Plugin Overwrite (Target is __init__.py, Source is ZIP)
                                    ...
                                    try:
                                        ...
                                        logging.info(
                                            "Plugin Installer: overwriting existing folder plugin: %s",
                                            target_dir,
                                        )
                                        ...
                                        with zipfile.ZipFile(download_path, "r") as z:
                                            for member in z.infolist():
                                                ...
                                                if not os.path.abspath(
                                                    ...
                                                    logging.warning(
                                                        "Plugin Installer: skipping suspicious zip entry (path traversal): %s",
                                                        member.filename,
                                                    )
                                                    continue
                                        ...
                                        if os.path.exists(settings_path):
                                            try:
                                                ...
                                                logging.warning(
                                                    "Plugin Installer: failed to backup settings.json (folder): %s",
                                                    e,
                                                )
                                        ...
                                        shutil.copytree(
                                            source, target_dir, dirs_exist_ok=True
                                        )
                                        ...
                                        if settings_backup:
                                            try:
                                                ...
                                                logging.info(
                                                    "Plugin Installer: restored settings.json (folder)"
                                                )
                                            except Exception as e:
                                                logging.warning(
                                                    "Plugin Installer: failed to restore settings.json (folder): %s",
                                                    e,
                                                )
                                        ...
                                        logging.warning(
                                            "Plugin Installer: manual folder overwrite failed: %s — falling back to manager",
                                            e,
                                        )
                            ...
                            if did_manual_overwrite:
                                    ...
                                    if os.path.basename(target_file) == "__init__.py":
                                        ...
                                        if os.path.exists(settings_json):
                                            try:
                                                ...
                                                logging.warning(
                                                    "Plugin Installer: failed to backup settings.json: %s",
                                                    e,
                                                )

                                self.main_window.plugin_manager.install_plugin(
                                    download_path
                                )
                                ...
                                if saved_settings_content and target_dir_for_restore:
                                    ...
                                    if os.path.exists(target_dir_for_restore):
                                        ...
                                        try:
                                            with open(
                                                settings_json, "w", encoding="utf-8"
                                            ) as f:
                                            ...
                                            logging.warning(
                                                "Plugin Installer: failed to restore settings.json: %s",
                                                e,
                                            )
                                ...
                                if target_file and os.path.exists(target_file):
                                    ...
                                    old_is_init = (
                                        os.path.basename(target_file) == "__init__.py"
                                    )
                                    ...
                                    if new_is_zip and old_is_py and not old_is_init:
                                        try:
                                            ...
                                            logging.warning(
                                                "Plugin Installer: failed to remove old .py during transition: %s",
                                                e,
                                            )
                                    ...
                                    # Case 2: Transition from Package (Folder) to Single File (.py)
                                        ...
                                        try:
                                            ...
                                            logging.warning(
                                                "Plugin Installer: failed to remove old folder during transition: %s",
                                                e,
                                            )
                            ...
                            if not getattr(self, "_batch_updating", False):
                                self.main_window.plugin_manager.discover_plugins(
                                    self.main_window
                                )
                                _refresh_plugin_menus(self.main_window)
