                ...
                if response.status == 200:
                    ...
                    if props:
                        ...
                        if "SMILES" in compound_data:
                            ...
                            return (
                                None,
                                f"SMILES data missing in response for '{clean_name}'",
                            )
                    else:
...
SETTINGS_FILE = os.path.join(
    os.path.dirname(__file__), "chat_with_molecule_neo_chatGPT.json"
)
LOG_FILE = os.path.join(os.path.dirname(__file__), "chat_with_molecule_neo_chatGPT.log")
...
def load_settings():
    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            ...
            logging.warning("[chat_with_molecule_neo_chatGPT.py:390] silenced: %s", _e)
    return {}
...
class ChatBrowser(QTextBrowser):
    ...
    def __init__(self, parent=None):
        ...
        self.setOpenExternalLinks(
            False
        )  # We handle everything in text_browser.anchorClicked
...
class InitWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Fetch models and filter for chat models (gpt-*)
            all_models = client.models.list()
            models = [m.id for m in all_models if "gpt" in m.id]
            # specific sort or prioritization could go here
            models.sort()
...
class NameResolverWorker(QRunnable):
    ...
    def run(self):
        try:
            ...
            logging.warning("[chat_with_molecule_neo_chatGPT.py:526] silenced: %s", _e)
...
class ChatMoleculeWindow(QDialog):
    def __init__(self, context):
        ...
        try:
            ...
            self.pending_context_msg = (
                None  # Pending context update to send with next user message
            )
            self.pending_info_text = (
                None  # Pending visual notification to show in chat on next send
            )
            ...
            QMessageBox.critical(
                self.main_window,
                "Plugin Init Error",
                f"Failed to initialize Chat Window:\n{e}",
            )
            raise e
        ...
        if is_first_check or has_changed:
            ...
            if smiles_changed:
                ...
                if current_smiles and Chem:
                    try:
                        ...
                        logging.warning(
                            "[chat_with_molecule_neo_chatGPT.py:682] silenced: %s", _e
                        )
            ...
            if current_smiles:
                ...
                self.lbl_context.setText(
                    f"Context: {self.get_molecule_name(allow_fetch=False) or 'Unknown'} ({current_smiles[:20]}...)"
                )
            else:
            ...
            if not is_first_check:
                if current_smiles:
                    ...
                    self.pending_context_msg = self._build_context_msg(
                        current_smiles, mol_name, lazy=True
                    )
                    ...
                    if current_selection:
                        ...
                        sel_info_html = (
                            " <span style='color:gray'>[Selection cleared]</span>"
                        )
                    ...
                    if smiles_changed:
                        ...
                        append_log(
                            "INFO",
                            f"Context updated: {current_smiles}, Sel: {current_selection}",
                        )
                    ...
                    # Molecule unloaded
                    self.pending_context_msg = (
                        "System Update: The user has unloaded the molecule."
                    )
                    if error:
    ...
    def init_ui(self):
        ...
        if icon_path and os.path.exists(icon_path):
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:760] silenced: %s", _e
                )
        ...
        self.txt_api_key.setPlaceholderText("Enter OpenAI API Key")
        ...
        lbl_link.setToolTip("Click to open OpenAI Platform and generate an API key")
        ...
        default_model = self.settings.get("model", "gpt-4o")
        ...
        self.tool_confirm_frame.setStyleSheet(
            "background-color: #f0f8ff; border: 1px solid #87ceeb; border-radius: 5px;"
        )
        tool_layout = QVBoxLayout(self.tool_confirm_frame)  # Changed to Vertical
        tool_layout.setContentsMargins(5, 5, 5, 5)

        self.lbl_tool_info = QLabel("ChatGPT wants to execute a command...")
        ...
        try:
            ...
            if self.worker and self.worker.isRunning():
                try:
                    ...
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:944] silenced: %s", _e
                    )
                try:
                    self.worker.response_received.disconnect()
                except Exception as _e:
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:946] silenced: %s", _e
                    )
                try:
                    self.worker.finished.disconnect()
                except Exception as _e:
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:948] silenced: %s", _e
                    )
                try:
                    self.worker.error_occurred.disconnect()
                except Exception as _e:
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:950] silenced: %s", _e
                    )

            if (
                getattr(self, "init_worker", None) is not None
                and self.init_worker
                and self.init_worker.isRunning()
            ):
                try:
                    self.init_worker.finished.disconnect()
                except Exception as _e:
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:954] silenced: %s", _e
                    )
    ...
    def fetch_models(self):
        """Fetch available ChatGPT models"""
        if DEMO_MODE:
    ...
    def save_settings(self):
        ...
        self.append_message(
            "System", "Settings saved and session re-initialized.", "green"
        )
    ...
    def handle_link(self, url):
        ...
        if scheme == "smiles":
            ...
            self.append_message(
                "System", f"Loading molecule from SMILES: {smiles} ...", "blue"
            )
            ...
            if hasattr(self.main_window, "string_importer_manager"):
                try:
                    ...
                    self.append_message(
                        "System", f"Failed to load molecule: {e}", "red"
                    )
            else:
                self.append_message(
                    "System",
    ...
    def render_content(self, text):
        ...
        processed_text = re.sub(
            r"(?<!\$)\$([^$]+)\$(?!\$)", replace_latex_inline, processed_text
        )
    ...
    def append_message(self, sender, text, color="black"):
        ...
        html = (
            f"{style}<b style='color:{color}'>{sender}:</b><br>{processed_text}<br><br>"
        )
    ...
    def export_history(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Success", f"Exported to {os.path.basename(file_path)}"
            )
        except Exception as e:
    ...
    def initialize_session(self):
        ...
        if DEMO_MODE:
            ...
            self.append_message(
                "System",
                "<b>DEMO MODE ACTIVE</b><br>No API Key required. Responses are simulated.",
                "orange",
            )
            # Enable UI
        ...
        if not HAS_OPENAI:
            self.append_message(
                "System", "Error: 'openai' library is not installed.", "red"
            )
        ...
        if not api_key:
            self.append_message(
                "System", "Please enter your OpenAI API Key above.", "orange"
            )
            QMessageBox.warning(
                self,
                "Welcome",
                "Please enter your OpenAI API Key in the settings above to start chatting.",
            )
    ...
    def on_init_finished(self, available_models, error_msg):
        ...
        if not available_models:
            self.append_message(
                "System", "No models found (or API Key invalid).", "red"
            )
            return
        ...
        if saved_model in available_models:
            ...
            # Prioritize efficient models as requested
            # We check for exact matches or partial matches for robustness
            preferred_defaults = ["gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo"]
            ...
            # Fallback: any gpt, then just the first available
            if not target_model_name:
                gpts = [m for m in available_models if "gpt" in m]
                if gpts:
                    target_model_name = gpts[0]
                else:
                    target_model_name = available_models[0]

            self.append_message(
                "System",
                f"Default/Saved model '{saved_model}' not found. Auto-selecting: {target_model_name}",
                "gray",
            )
        ...
        # --- Start Chat ---
        self.append_message(
            "System", f"Initializing chat with: {target_model_name}", "gray"
        )
        ...
        try:
            ...
            self.append_message(
                "System", f"Error initializing OpenAI client: {e}", "red"
            )
            self.client = None
            return
        ...
        if smiles and Chem:
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:1331] silenced: %s", _e
                )
        ...
        if smiles:
            ...
            context_msg = self._build_context_msg(
                smiles, mol_name
            )  # Call the class methodhelper
            self.pending_context_msg = (
                context_msg  # Store to send with first user message
            )
            self.pending_context_msg = (
                context_msg  # Store to send with first user message
            )
            ...
            context_msg = "No molecule is currently loaded. "
            if error:
    ...
    def get_current_molecule_smiles(self):
        ...
        try:
            if hasattr(self.main_window, "data") and hasattr(
                self.main_window.data, "to_rdkit_mol"
            ):
                if (
                    hasattr(self.main_window.data, "atoms")
                    and self.main_window.data.atoms
                ):
                    ...
                    try:
                        ...
                        for aid in sorted_ids:
                            ...
                            a.SetAtomMapNum(
                                aid + 1
                            )  # KEY FIX: Set MapNum here directly!

                            if "charge" in atom_data:
                                a.SetFormalCharge(atom_data["charge"])
                            if "explicit_valence" in atom_data:
                                a.SetNumExplicitHs(atom_data["explicit_valence"])
                        ...
                        # 2. Add Bonds
                        if (
                            hasattr(self.main_window.data, "bonds")
                            and self.main_window.data.bonds
                        ):
                            sorted_bids = sorted(
                                self.main_window.data.bonds.keys(), key=str
                            )
                            for bid in sorted_bids:
                                ...
                                if id1 in id_map and id2 in id_map:
                                    ...
                                    bond_idx = rwvm.AddBond(
                                        id_map[id1], id_map[id2], btype
                                    )
        ...
        if mol is None:
            if (
                ...
                and hasattr(self.main_window.data, "atoms")
                and not self.main_window.data.atoms
            ):
    ...
    def load_smiles_undo_safe(self, smiles_string):
        ...
        try:
            ...
            if mol is None:
                self.append_message(
                    "System",
                    f"Error: Generated SMILES is invalid.\nInput: {cleaned_smiles[:50]}...",
                    "red",
                )
                return
            ...
            # --- RECONSTRUCT SCENE ---
            SCALE_FACTOR = 50.0
            view_center = mw.init_manager.view_2d.mapToScene(
                mw.init_manager.view_2d.viewport().rect().center()
            )
            ...
            for i in range(mol.GetNumAtoms()):
                ...
                atom_id = mw.init_manager.scene.create_atom(
            ...
            for bond in mol.GetBonds():
                ...
                if b_idx in rdkit_idx_to_my_id and e_idx in rdkit_idx_to_my_id:
                    ...
                    a1_item = mw.scene.atom_items.get(a1_id)
                    a2_item = mw.scene.atom_items.get(a2_id)
                    mw.init_manager.scene.create_bond(
            ...
            if mw.state_manager.data.atoms:
                mw.init_manager.view_2d.fitInView(
                    mw.init_manager.scene.sceneRect(),
            ...
            try:
                self.context.check_chemistry_problems()
            except Exception as _e:
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:1628] silenced: %s", _e
                )
            ...
            if hasattr(mw.init_manager, "scene"):
                mw.init_manager.scene.update()
            if hasattr(mw.init_manager, "view_2d"):
            ...
            if not mol or mol.GetNumConformers() == 0:
                ...
                if mol:
                    try:
                        ...
                        logging.warning(
                            "[chat_with_molecule_neo_chatGPT.py:1721] silenced: %s", _e
                        )
    ...
    def propose_tool_action(self, payload):
        ...
        if is_multiple:
            ...
            self.btn_tool_step.setStyleSheet(
                f"background-color: {BTN_COLOR_ACCEPT_SINGLE}"
            )
            self.btn_tool_step.setVisible(True)
            self.btn_tool_accept.setText("Accept All")
            self.btn_tool_accept.setStyleSheet(
                f"background-color: {BTN_COLOR_ACCEPT_ALL}"
            )
        ...
        self.txt_input.setEnabled(
            False
        )  # Freeze input while tool confirmation is shown
    ...
    def on_tool_step_click(self):
        ...
        if not self.pending_tool_payload:
            return
        ...
        if error:
            ...
            self.lbl_tool_info.setText(
                f"Tool '{tool_name}' failed during step: {error}"
            )
        ...
        if not tools_list:
            ...
            self.append_message(
                "System", "Step completed. Waiting for next...", "green"
            )
    ...
    def accept_tool_action(self):
        ...
        if "tools" in payload:
            tools_list = payload["tools"]
            self.append_message(
                "System", f"Executing {len(tools_list)} operations in order...", "blue"
            )
            ...
            for i, tool_item in enumerate(tools_list):
                ...
                self.append_message(
                    "System", f"[{i + 1}/{len(tools_list)}] {tool_name}...", "gray"
                )
                QApplication.processEvents()
    ...
    def execute_apply_transformation(self, params):
        ...
        try:
            ...
            if not reaction_smarts:
                self.append_message(
                    "System", "Error: No reaction SMARTS provided.", "red"
                )
                return
            ...
            if not Chem or not AllChem:
                self.append_message(
                    "System", "RDKit is required for this feature.", "red"
                )
                return
            ...
            if not mol:
                self.append_message(
                    "System", "Error: Could not parse current molecule.", "red"
                )
                return
            ...
            if not products:
                ...
                self.append_message(
                    "System",
                    "Reaction produced no products with explicit Hs. Checking implicit...",
                    "orange",
                )
                ...
                if not products:
                    self.append_message(
                        "System",
                        "Reaction produced no products. Check compatibility.",
                        "orange",
                    )
                    return
            ...
            if target_index is not None:
                try:
                    ...
                    matches = mol_with_h.GetSubstructMatches(
                        reactant_template, uniquify=False
                    )
                    ...
                    if candidate_indices:
                        ...
                        self.append_message(
                            "System",
                            f"Selected match #{best_idx} (Atoms: {best_count}) from {len(candidate_indices)} candidates.",
                            "gray",
                        )
                    else:
                        self.append_message(
                            "System",
                            "Warning: Selected atom not found in reaction matches. Applying to first match.",
                            "orange",
                        )
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2166] silenced: %s", _e
                )
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2170] silenced: %s", _e
                )
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2174] silenced: %s", _e
                )
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2185] silenced: %s", _e
                )
            ...
            self.append_message(
                "System", f"Transformation Applied.\nRule: `{reaction_smarts}`", "green"
            )

        except Exception as e:
            self.append_message(
                "System",
                f"Transformation Failed: {e}\n(Tip: The SMARTS pattern might be invalid. Ask ChatGPT for a more precise one.)",
                "red",
            )
            return str(e)
    ...
    def execute_highlight_substructure(self, params):
        ...
        try:
            ...
            if smarts:
                ...
                if not query:
                    ...
                    if matches:
                        ...
                        self.append_message(
                            "System", f"No matches found for: {smarts}", "orange"
                        )
                        return
            ...
            count = 0
            if hasattr(self.main_window.state_manager, "data") and hasattr(
                self.main_window.state_manager.data, "atoms"
            ):
    ...
    def execute_set_electronic_state(self, params):
        ...
        try:
            ...
            mol = Chem.MolFromSmiles(current_smiles)
            if not mol:
                return "Could not parse current molecule."
            ...
            if not changes_made and (new_charge is not None or new_mult is not None):
                ...
                msg = []
                if new_charge is not None and hasattr(
                    self.main_window.state_manager, "current_charge"
                ):
                    self.main_window.state_manager.current_charge = int(new_charge)
                    msg.append(f"Global Charge={new_charge}")
                if new_mult is not None and hasattr(
                    self.main_window.state_manager, "current_mult"
                ):
                ...
                if msg:
                    ...
                    self.append_message(
                        "System",
                        f"Updated Global Metadata: {', '.join(msg)} (Atoms unchanged)",
                        "orange",
                    )
                    return
            ...
            if changes_made:
                ...
                if atom_indices_param:
                    debug_msg += f"Requested: {atom_indices_param}. "
                if target_smarts:
                    debug_msg += f"SMARTS: {target_smarts}. "

                self.append_message(
                    "System",
                    f"No atoms targeted. {debug_msg} Select atoms or use SMARTS.",
                    "orange",
                )
    ...
    def execute_calculate_descriptors(self, params):
        ...
        try:
            ...
            if current_smiles:
                self.calc_results_by_smiles[current_smiles] = (
            ...
            self.append_message(
                "System",
                f"Calculation Error: {e}\n(Tip: Ask ChatGPT to ensure the molecule is valid.)",
                "red",
            )
            return str(e)
    ...
    def execute_load_molecule(self, params):
        ...
        try:
            ...
            if name:
                ...
                self.append_message(
                    "System", f"Molecule loaded from SMILES: {smiles}", "green"
                )

        except Exception as e:
            self.append_message(
                "System",
                f"Load Error: {e}\n(Tip: The SMILES string might be invalid. Ask ChatGPT for a more precise one.)",
                "red",
            )
            return str(e)
    ...
    def execute_load_molecule_by_name(self, params):
        ...
        try:
            ...
            if not name:
                self.append_message(
                    "System", "Error: No name provided for search.", "red"
                )
    ...
    def execute_convert_to_3d(self, params):
        ...
        try:
            ...
            self.append_message(
                "System",
                f"3D Conversion Error: {e}\n(Tip: Ask ChatGPT to try again.)",
                "red",
            )
            return str(e)
    ...
    def _get_molecule_for_export(self):
        ...
        if not mol_to_export:
            self.append_message(
                "System", "Triggering 3D conversion for input...", "blue"
            )
            self._ensure_main_window_3d_conversion()
        ...
        if not mol_to_export:
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2674] silenced: %s", _e
                )
            self.append_message(
                "System",
                f"Main Window conversion failed (Status: {status_msg}). Trying local generation...",
                "orange",
            )
            ...
            try:
                ...
                if mol_3d:
                    ...
                    if res == 0:
                        ...
                        self.append_message(
                            "Error",
                            f"Local 3D Embed failed (Result code: {res})",
                            "red",
                        )
                else:
                    self.append_message(
                        "Error", "Local MolFromSmiles failed (Invalid SMILES?)", "red"
                    )
            except Exception as e:
    ...
    def _save_input_file(self, filename, content, filter_str):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Input File", filename, filter_str
        )
        if file_path:
            try:
                ...
                self.append_message(
                    "System",
                    f"Input file saved to {os.path.basename(file_path)}",
                    "green",
                )
                return None
            except Exception as e:
    ...
    def execute_orca_input_generator(self, params):
        ...
        if not mol:
            self.append_message(
                "Error", "Could not generate 3D structure. Input file cancelled.", "red"
            )
        ...
        try:
            ...
            for i, atom in enumerate(mol.GetAtoms()):
                pos = conf.GetAtomPosition(i)
                content += (
                    f"{atom.GetSymbol():2s} {pos.x:10.5f} {pos.y:10.5f} {pos.z:10.5f}\n"
                )
            content += "*\n"
            content += footer + "\n"

            return self._save_input_file(
                filename, content, "ORCA Input (*.inp *.orca);;All Files (*)"
            )
        except Exception as e:
    ...
    def execute_gaussian_input_generator(self, params):
        ...
        try:
            ...
            return self._save_input_file(
                filename, content, "Gaussian Input (*.gjf *.com);;All Files (*)"
            )
        except Exception as e:
    ...
    def execute_save_file(self, params):
        ...
        if any(tag in content for tag in tags):
            ...
            if not mol:
                self.append_message(
                    "Error", "Geometry tag used but no valid molecule found.", "red"
                )
            ...
            try:
                ...
                self.append_message(
                    "Error", f"Failed to generate geometry block: {e}", "red"
                )
                return str(e)
    ...
    def _build_context_msg(self, smiles, name, lazy=False):
        ...
        try:
            ...
            logging.warning("[chat_with_molecule_neo_chatGPT.py:2867] silenced: %s", _e)
    ...
    def stop_generation(self):
        ...
        if self.worker and self.worker.isRunning():
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2890] silenced: %s", _e
                )
            try:
                self.worker.response_received.disconnect()
            except Exception as _e:
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2892] silenced: %s", _e
                )
            try:
                self.worker.finished.disconnect()
            except Exception as _e:
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2894] silenced: %s", _e
                )
            try:
                self.worker.error_occurred.disconnect()
            except Exception as _e:
                logging.warning(
                    "[chat_with_molecule_neo_chatGPT.py:2896] silenced: %s", _e
                )
    ...
    def send_message(self):
        ...
        if not self.client:
            ...
            self.append_message(
                "System", "Chat session not initialized. Please wait...", "orange"
            )
            return
        ...
        if DEMO_MODE:
            ...
            # Simulate delay
            QTimer.singleShot(
                1500, lambda: self.on_final_response("Demo Response: This is a test.")
            )
            return
        ...
        try:
            ...
            logging.warning("[chat_with_molecule_neo_chatGPT.py:2962] silenced: %s", _e)
        ...
        if current_smiles:
            ...
            if not mol_name and self.last_inchikey and PubChemResolver:
                ...
                QApplication.processEvents()
                try:
                    name_res, _ = PubChemResolver.resolve_inchikey_to_name(
                        self.last_inchikey
                    )
                    if name_res:
                        if getattr(self, "_name_cache", None) is None:
                        ...
                        smiles_preview = (
                            f"({current_smiles[:20]}...)" if current_smiles else ""
                        )
                        self.lbl_context.setText(
                            f"Context: {mol_name} {smiles_preview}"
                        )
                except Exception as _e:
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:2991] silenced: %s", _e
                    )
            ...
            self.pending_context_msg = self._build_context_msg(
                current_smiles, mol_name, lazy=False
            )
        ...
        self.worker = OpenAIWorker(
            self.client, self.chat_history_state, model_name=model
        )
    ...
    def _ensure_main_window_3d_conversion(self):
        ...
        try:
            self.append_message(
                "System", "Converting 2D structure to 3D... (Max 60s)", "blue"
            )
            QApplication.processEvents()
            ...
            try:
                ...
                self.append_message(
                    "Error", f"Exception during trigger_conversion: {e}", "red"
                )
                return
            ...
            if prev_conversion_id != -1 and curr_conversion_id == prev_conversion_id:
                ...
                try:
                    ...
                    logging.warning(
                        "[chat_with_molecule_neo_chatGPT.py:3244] silenced: %s", _e
                    )
                self.append_message(
                    "Error", f"3D Conversion failed to start: {status_msg}", "red"
                )
                return
        ...
        try:
            ...
            if current_points:
                ...
                if hasattr(mw, "init_manager") and mw.init_manager.view_2d:
                    target_center = mw.init_manager.view_2d.mapToScene(
                        mw.init_manager.view_2d.viewport().rect().center()
                    )
                else:
            ...
            if not hasattr(mw.state_manager.data, "next_atom_id"):
                if mw.state_manager.data.atoms:
                    ...
                    mw.state_manager.data.next_atom_id = (
                        max(mw.state_manager.data.atoms.keys()) + 1
                    )
                else:
            ...
            for i in range(mol.GetNumAtoms()):
                ...
                if map_num > 0:
                    ...
                    if target_id in mw.state_manager.data.atoms:
                        ...
                        actual_id = mw.scene.create_atom(
                            atom.GetSymbol(),
                        ...
                        if hasattr(mw.state_manager.data, "next_atom_id"):
                            current_max = (
                                max(mw.state_manager.data.atoms.keys())
                                if mw.state_manager.data.atoms
                                else 0
                            )
            ...
            for bond in mol.GetBonds():
                ...
                if a1_map > 0 and a2_map > 0:
                    ...
                    if (
                        id1 in mw.state_manager.data.atoms
                        and id2 in mw.state_manager.data.atoms
                    ):
                        ...
                        found_key = None
                        if (id1, id2) in mw.state_manager.data.bonds:
                            found_key = (id1, id2)
                        elif (id2, id1) in mw.state_manager.data.bonds:
                            found_key = (id2, id1)
                        ...
                        if found_key:
                            ...
                            if (
                                b_data.get("order", None) != order
                                or b_data.get("stereo", 0) != stereo
                            ):
                                b_data["order"] = order
                                ...
                                if item:
                                    ...
                                    if hasattr(item, "set_order"):
                                        item.set_order(order)
                                    if hasattr(item, "set_stereo"):
                                        item.set_stereo(stereo)
                                    item.update()
                            ...
                            new_key = mw.scene.create_bond(
                                item1, item2, bond_order=order, bond_stereo=stereo
                            )
                            if new_key:
                                ...
                                if (id1, id2) in mw.state_manager.data.bonds:
                                    current_step_bond_keys.add((id1, id2))
                                elif (id2, id1) in mw.state_manager.data.bonds:
                                    current_step_bond_keys.add((id2, id1))
