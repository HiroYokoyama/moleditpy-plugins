...
class MoleculeResolverDialog(QDialog):
                ...
                if response.status_code == 200:
                    ...
                    for p in props:
                        ...
                        self.context.show_status_message(
                            f"PubChem: Identifying structure..."
                        )
                        results.append(
                            {
        ...
        if network_error:
            ...
            if results:
                self.lbl_info.setText(
                    f"Found {len(results)} candidates. Select one and click Load to 2D Editor."
                )
            else:
    ...
    def load_molecule(self):
        ...
        if not selected_items:
            QMessageBox.warning(
                self, PLUGIN_NAME, "Please select a molecule from the list."
            )
            return
        ...
        if not smiles:
            QMessageBox.warning(
                self, PLUGIN_NAME, "No SMILES data available for this entry."
            )
            return
