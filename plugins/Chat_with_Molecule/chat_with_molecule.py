...
def latex_to_html(latex_str):
    ...
    try:
        ...
        content = content.replace(
            r"\text", r"\mathrm"
        )  # \mathrm is safer than \text in some mpl versions
        ...
        if is_first_check or has_changed:
            ...
            if not is_first_check:
                if current_smiles:
                    ...
                    append_log(
                        "INFO", f"Context auto-updated (queued): {current_smiles}"
                    )
    ...
    def handle_link(self, url):
        ...
        if scheme == "smiles":
            ...
            if hasattr(self.main_window, "main_window_string_importers"):
                try:
                    self.main_window.main_window_string_importers.load_from_smiles(
                        smiles
                    )
        ...
        try:
            ...
            if models:
                ...
                if index >= 0:
                    ...
                    if geminis:
                        self.combo_model.setCurrentText(geminis[0])
            else:
                self.append_message(
                    "System",
                    "No models found with 'generateContent' capability.",
                    "orange",
                )
