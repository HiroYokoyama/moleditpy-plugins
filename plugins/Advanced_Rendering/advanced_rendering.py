...
def initialize(context):
    ...
    context.add_menu_action(
        "Settings/Advanced Graphics Settings", lambda: load_plugin_from_mw(mw)
    )
...
class AdvancedGraphicsWidget(QWidget):
    ...
    def _retry_init_lighting(self):
        ...
        if ready:
            ...
            if self._init_attempts < self._max_init_attempts:
                ...
                logging.warning(
                    "Advanced Rendering: Timed out waiting for renderer initialization."
                )
    ...
    def get_settings_path(self):
        ...
        return os.path.join(
            ...
            "advanced_graphics_settings.json",
        )
    ...
    def update_atoms_pbr(self, force_pbr=False):
        ...
        plotter = self.safe_plotter
        if not plotter or not hasattr(plotter, "renderer"):
            return
    ...
    def on_load_env_texture(self):
        fname, _ = QFileDialog.getOpenFileName(
            self, "Open Texture", "", "Images (*.png *.jpg *.jpeg *.hdr)"
        )
        if fname:
    ...
    def on_shadows_toggled(self, checked):
        ...
        if (
            checked
            and "(Advanced Rendering)" not in curr_style
            and not self.use_atom_pbr
        ):
    ...
    def apply_settings_dict(self, data):
        ...
        if "use_shadows" in data:
            self.check_shadows.setChecked(data["use_shadows"])
        ...
        if "use_ssao" in data:
            self.check_ssao.setChecked(data["use_ssao"])
        if "use_depth_peeling" in data:
            self.check_depth.setChecked(data["use_depth_peeling"])
        if "use_aa" in data:
        ...
        if "use_atom_pbr" in data:
            self.check_atom_pbr.setChecked(data["use_atom_pbr"])
