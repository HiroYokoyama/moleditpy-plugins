...
class ChatMoleculeWindow(QDialog):
    ...
    def get_current_molecule_smiles(self):
        ...
        try:
            if hasattr(self.state_manager, 'data') and hasattr(self.state_manager.data, 'to_rdkit_mol'):

                if hasattr(self.state_manager.data, 'atoms') and self.state_manager.data.atoms:
                    ...
                    try:
                        ...
                        # 2. Add Bonds
                        if hasattr(self.state_manager.data, 'bonds') and self.state_manager.data.bonds:
                            sorted_bids = sorted(self.state_manager.data.bonds.keys(), key=str)
    ...
    def load_smiles_undo_safe(self, smiles_string):
        ...
        try:
            ...
            self.init_manager.analysis_action.setEnabled(False)
            ...
            for i in range(mol.GetNumAtoms()):
                ...
                atom_id = scene.create_atom(atom.GetSymbol(), QPointF(scene_x, scene_y), charge=charge)
            ...
            # Force Scene Update
            if hasattr(self.init_manager, 'scene'): self.init_manager.scene.update()
            if hasattr(self.init_manager, 'view_2d'): self.init_manager.view_2d.viewport().update()
    ...
    def _get_descriptors_str(self, smiles):
        ...
        try:
            ...
            mol = None
            if hasattr(self.view_3d_manager, 'current_mol') and self.view_3d_manager.current_mol:
                mol = self.view_3d_manager.current_mol
    ...
    def execute_highlight_substructure(self, params):
        ...
        try:
            ...
            count = 0
            if hasattr(self.state_manager, 'data') and hasattr(self.state_manager.data, 'atoms'):
                ...
                for aid in highlight_ids:
                    ...
                    if aid in self.state_manager.data.atoms:
                        item = self.state_manager.data.atoms[aid].get('item')
                        if item:
