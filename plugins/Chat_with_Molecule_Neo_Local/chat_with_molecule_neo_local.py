...
SETTINGS_FILE = os.path.join(
    os.path.dirname(__file__), "chat_with_molecule_neo_local.json"
)
...
class ChatMoleculeWindow(QDialog):
    def __init__(self, context):
        ...
        try:
            ...
            self.calc_results_by_smiles = {}  # Molecular memory for calculation results
            ...
            # V3 Managers for easier access
            self.init_manager = self.main_window.init_manager
            self.state_manager = self.main_window.state_manager
            self.edit_actions_manager = self.main_window.edit_actions_manager
            self.view_3d_manager = self.main_window.view_3d_manager
            self.compute_manager = getattr(self.main_window, "compute_manager", None)
    ...
    def init_ui(self):
        ...
        if icon_path and os.path.exists(icon_path):
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_local.py:783] silenced: %s", _e
                )
        ...
        self.chk_enable_pubchem.setChecked(
            not self.settings.get("disable_pubchem", True)
        )
    ...
    def check_settings_changed(self):
        ...
        saved_model = self.settings.get(
            "model", "local-model"
        )  # default fallback match init
        ...
        has_changes = (
            ...
            or current_api_key != saved_api_key
            or current_model != saved_model
            or current_disable_pubchem != saved_disable_pubchem
        )
    ...
    def _get_privacy_details(self):
        ...
        if is_loopback:
            ...
            if disable_pubchem:
                notice_color = "green"
                privacy_text = (
                    "Privacy Notice: Chat is local. InChIKey is NOT sent to PubChem."
                )
            else:
                notice_color = "orange"
                privacy_text = (
                    "Privacy Notice: Chat is local. Only InChIKey is sent to PubChem."
                )
    ...
    def save_settings(self):
        ...
        self.append_message(
            "System",
            f"Settings saved and session re-initialized.<br><span style='color:gray; font-size:0.9em;'>({privacy_text})</span>",
            "green",
        )
    ...
    def initialize_session(self):
        ...
        if not api_base:
            ...
            self.append_message(
                "System", "Please enter a Local API Base URL above to start.", "orange"
            )
    ...
    def get_current_molecule_smiles(self):
        ...
        try:
            if hasattr(self.state_manager, "data") and hasattr(
                self.state_manager.data, "to_rdkit_mol"
            ):
                if (
                    hasattr(self.state_manager.data, "atoms")
                    and self.state_manager.data.atoms
                ):
                    ...
                    try:
                        ...
                        sorted_ids = sorted(self.state_manager.data.atoms.keys())
                        for aid in sorted_ids:
                            atom_data = self.state_manager.data.atoms[aid]
                        ...
                        # 2. Add Bonds
                        if (
                            hasattr(self.state_manager.data, "bonds")
                            and self.state_manager.data.bonds
                        ):
                            sorted_bids = sorted(
                                self.state_manager.data.bonds.keys(), key=str
                            )
                            for bid in sorted_bids:
                                bond_data = self.state_manager.data.bonds[bid]
        ...
        if mol is None:
            if (
                hasattr(self.state_manager, "data")
                and hasattr(self.state_manager.data, "atoms")
                and not self.state_manager.data.atoms
            ):
    ...
    def load_smiles_undo_safe(self, smiles_string):
        ...
        try:
            ...
            # --- RECONSTRUCT SCENE ---
            SCALE_FACTOR = 50.0
            view_2d = self.init_manager.view_2d
            view_center = view_2d.mapToScene(view_2d.viewport().rect().center())
            ...
            for i in range(mol.GetNumAtoms()):
                ...
                if map_num > 0:
                    ...
                    if atom_id != target_id:
                        if target_id not in self.state_manager.data.atoms:
                            # Remap
                            try:
                                adata = self.state_manager.data.atoms.pop(atom_id)
                                self.state_manager.data.atoms[target_id] = adata
                                atom_id = target_id
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_local.py:1819] silenced: %s", _e
                )
            ...
            if hasattr(self.init_manager, "scene"):
                self.init_manager.scene.update()
            if hasattr(self.init_manager, "view_2d"):
                self.init_manager.view_2d.viewport().update()
    ...
    def on_tool_step_click(self):
        ...
        if error:
            ...
            self.btn_tool_accept.setText(
                f"Retry Step (Ask {self.combo_model.currentText()})"
            )
    ...
    def accept_tool_action(self):
        ...
        if error:
            ...
            self.btn_tool_accept.setText(
                f" Retry (Ask {self.combo_model.currentText()}) "
            )
    ...
    def execute_apply_transformation(self, params):
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_local.py:2361] silenced: %s", _e
                )
            ...
            self.append_message(
                "System",
                f"Transformation Failed: {e}\n(Tip: The SMARTS pattern might be invalid. Ask {self.combo_model.currentText()} for a more precise one.)",
                "red",
            )
            return str(e)
    ...
    def execute_highlight_substructure(self, params):
        ...
        try:
            ...
            if hasattr(self.state_manager, "data") and hasattr(
                ...
                if hasattr(self.init_manager, "scene"):
                    self.init_manager.scene.clearSelection()
    ...
    def execute_set_electronic_state(self, params):
        ...
        try:
            ...
            elif hasattr(self.main_window, "data") and hasattr(
            ...
            if not changes_made and (new_charge is not None or new_mult is not None):
                ...
                if new_charge is not None and hasattr(
                    self.main_window, "current_charge"
                ):
    ...
    def execute_calculate_descriptors(self, params):
        ...
        try:
            ...
            self.append_message(
                "System",
                f"Calculation Error: {e}\n(Tip: Ask {self.combo_model.currentText()} to ensure the molecule is valid.)",
                "red",
            )
            return str(e)
    ...
    def execute_load_molecule(self, params):
        ...
        try:
            ...
            self.append_message(
                "System",
                f"Load Error: {e}\n(Tip: The SMILES string might be invalid. Ask {self.combo_model.currentText()} for a more precise one.)",
                "red",
            )
            return str(e)
    ...
    def execute_convert_to_3d(self, params):
        ...
        try:
            ...
            self.append_message(
                "System",
                f"3D Conversion Error: {e}\n(Tip: Ask {self.combo_model.currentText()} to try again.)",
                "red",
            )
            return str(e)
    ...
    def _get_molecule_for_export(self):
        ...
        if not mol_to_export:
            ...
            try:
                ...
                logging.warning(
                    "[chat_with_molecule_neo_local.py:2865] silenced: %s", _e
                )
            self.append_message(
                "System",
    ...
    def send_message(self):
        ...
        if current_smiles:
            ...
            if (
                not mol_name
                and self.last_inchikey
                and PubChemResolver
                and not disable_pubchem
            ):
                ...
                try:
                    ...
                    logging.warning(
                        "[chat_with_molecule_neo_local.py:3129] silenced: %s", _e
                    )
        ...
        try:
            ...
            if prev_conversion_id != -1 and curr_conversion_id == prev_conversion_id:
                ...
                try:
                    ...
                    logging.warning(
                        "[chat_with_molecule_neo_local.py:3433] silenced: %s", _e
                    )
                self.append_message(
    ...
    def on_error(self, error_msg):
        ...
        self.append_message(
            "System", f"{self.combo_model.currentText()} Error: {error_msg}", "red"
        )
    ...
    def on_final_response(self, response):
        ...
        self.chat_history_log.append(
            {
                "sender": self.combo_model.currentText(),
...
def run(main_window):
    ...
    # Standard V3/Legacy Hybrid Launch
    if not hasattr(main_window, "plugin_manager"):
        return
