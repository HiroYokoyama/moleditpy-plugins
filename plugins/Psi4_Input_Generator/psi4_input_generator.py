...
class Psi4SetupDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.method_combo.addItems(
            ["scf", "b3lyp", "pbe", "mp2", "ccsd(t)", "wB97X-D", "m06-2x"]
        )
    ...
    def generate_content(self, filename_hint="job"):
        ...
        lines.append(
            f"{self.charge_spin.value()} {self.mult_spin.value()}"
        )  # Charge Mult
    ...
    def save_file(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save PSI4 Input", default_name, "PSI4 Input (*.dat *.in)"
        )
        if path:
            try:
                ...
                if "psi4.core.set_output_file" in content:
                    content = re.sub(
                        r"psi4\.core\.set_output_file\s*\(.*?\)", out_line, content
                    )
                else:
