    ...
    def run(self):
        ...
        try:
            ...
            for i, (orig_idx, probe_mol) in enumerate(self.targets):
                ...
                result_entry = {"index": orig_idx, "mol": probe_work, "rms": -1.0}
                ...
                if self.method == "Atom IDs":
                    if self.ignore_hs:
                        probe_heavy = [
                            a.GetIdx()
                            for a in probe_work.GetAtoms()
                            if a.GetAtomicNum() != 1
                        ]
                        ref_heavy = [
                            a.GetIdx()
                            for a in self.ref_work.GetAtoms()
                            if a.GetAtomicNum() != 1
                        ]
                        ...
                        if len(probe_heavy) == len(ref_heavy):
                            ...
                            try:
                                ...
                                logging.warning(
                                    "[molecule_comparator.py:87] silenced: %s", _e
                                )
                    else:
                        if probe_work.GetNumAtoms() == self.ref_work.GetNumAtoms():
                            ...
                            try:
                                ...
                                logging.warning(
                                    "[molecule_comparator.py:96] silenced: %s", _e
                                )
                ...
                # --- Method B: MCS (修正箇所) ---
                    ...
                    if res.numAtoms > 0:
                        ...
                        probe_matches = probe_calc.GetSubstructMatches(
                            patt, uniquify=False
                        )
                        ...
                        if ref_matches and probe_matches:
                            ...
                            if total_combos > self.MAX_COMBINATIONS:
                                print(
                                    f"[WARNING] Mol {orig_idx}: Too many MCS matches ({total_combos} combinations)."
                                )
                                print(
                                    f"          Search will be limited to {self.MAX_COMBINATIONS} iterations."
                                )
                                print(
                                    f"          Consider using 'Atom IDs' fitting method for better performance."
                                )
                            ...
                            for ref_match in ref_matches:
                                ...
                                for probe_match in probe_matches:
                                    ...
                                    if combo_count % 50 == 0:
                                        ...
                                        max_value = min(
                                            total_combos, self.MAX_COMBINATIONS
                                        )
                                        progress_pct = int(
                                            combo_count / max_value * 100
                                        )
                                        self.progress.emit(
                                            i,
                                            total_steps,
                                            f"Aligning Mol {orig_idx + 1}: {progress_pct}% ({combo_count}/{max_value})",
                                        )
...
class MoleculeComparator(QWidget):
    ...
    def init_ui(self):
        ...
        self.check_wireframe_lighting.setToolTip(
            "Enable or disable lighting when Wireframe style is selected"
        )
        self.check_wireframe_lighting.stateChanged.connect(
        ...
        self.table_results.customContextMenuRequested.connect(
    ...
    def export_molecule_coords(self, row, format_type):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self.mw,
            f"Export {name} as {format_type.upper()}",
            default_name,
            file_filter,
        )
    ...
    def on_selection_changed(self, row):
        ...
        self.btn_color.setStyleSheet(
            f"background-color: {entry['color']}; color: {text_color};"
        )
    ...
    def run_alignment(self):
        ...
        self.progress_dialog = QProgressDialog(
            "Initializing alignment...", "Stop", 0, len(target_entries), self.mw
        )
        ...
        self.worker = AlignmentWorker(
            ref_entry["mol"], target_entries, method, ignore_hs
        )
    ...
    def update_results_table(self):
        ...
        for i, entry in enumerate(self.molecules):
            ...
            rms_str = (
                f"{rms_val:.4f}"
                if rms_val is not None and rms_val >= 0
                else ("N/A" if rms_val == -1.0 else "-")
            )
    ...
    def update_visualization(self):
        ...
        self.context.draw_molecule_3d(combined_mol)
    ...
    def change_style(self, style_name):
        ...
        try:
            for action in self._find_style_actions():
                txt = action.text().replace("&", "")
                match = False
                if style_name == "CPK" and "CPK" in txt:
                    match = True
                elif style_name == "Wireframe" and "Wireframe" in txt:
                    match = True
                elif (
                    (style_name == "Sticks" or style_name == "Stick")
                    and "Stick" in txt
                    and "Ball" not in txt
                ):
                    match = True
                elif (
                    (style_name == "Ball and Stick" or style_name == "Ball & Stick")
                    and "Ball" in txt
                    and "Stick" in txt
                ):
                    match = True
    ...
    def exit_3d_only_mode(self):
        ...
        if (
            getattr(self, "saved_splitter_sizes", None) is not None
            and self.saved_splitter_sizes
        ):
            if hasattr(self.mw, "splitter"):
