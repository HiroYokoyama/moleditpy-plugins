    ...
    def get_compound_details(inchikey):
        ...
        try:
            ...
            with urllib.request.urlopen(prop_url) as response:
                ...
                if props:
                    p = props[0]
                    if p.get("MolecularFormula", None):
                        details["Formula"] = p.get("MolecularFormula", None)
                    if p.get("MolecularWeight", None):
                        details["Mol. Weight"] = p.get("MolecularWeight", None)
                    if p.get("IUPACName", None):
                        details["IUPAC Name"] = p.get("IUPACName", None)
...
class MoleculeDetailsDialog(QDialog):
    ...
    def copy_to_clipboard(self):
        ...
        order = ["Common Name", "IUPAC Name", "Formula", "Mol. Weight", "InChIKey"]
...
def resolve_and_load(context):
    ...
    if error:
        QMessageBox.warning(
            mw, "PubChem Error", f"Could not resolve '{name}':\n{error}"
        )
        return
...
def identify_current_molecule(context):
    ...
    if details:
        ...
        QMessageBox.information(
            mw, "PubChem Result", f"Could not identify molecule.\nError: {error}"
        )
