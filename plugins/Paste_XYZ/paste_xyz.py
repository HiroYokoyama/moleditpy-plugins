...
class PasteXYZDialog(QDialog):
    def __init__(self, parent=None):
        ...
        info_label = QLabel(
            "Paste XYZ coordinates below (Header/Atom count is IGNORED)."
        )
        ...
        try:
            ...
            def _process_with_charge(charge_val):
                ...
                if hasattr(mw, "edit_actions_manager") and hasattr(
                    mw.edit_actions_manager, "apply_chem_check_and_set_flags"
                ):
                    mw.edit_actions_manager.apply_chem_check_and_set_flags(
                        candidate_mol, source_desc="PasteXYZ"
                    )
            ...
            final_mol = None
            settings = (
                getattr(mw.init_manager, "settings", {})
                                ...
                                if skip_flag:
                                    if hasattr(mw, "io_manager") and hasattr(
                                        ...
                                        try:
                                            ...
                                            logging.warning(
                                                "[paste_xyz.py:193] silenced: %s", _e
                                            )
                                    final_mol = mol.GetMol()
                                    final_mol.SetIntProp("_xyz_skip_checks", 1)
                                    break
                            ...
                            if skip_flag:
                                if hasattr(mw, "io_manager") and hasattr(
                                    ...
                                    try:
                                        ...
                                        logging.warning(
                                            "[paste_xyz.py:209] silenced: %s", _e
                                        )
                                final_mol = mol.GetMol()
                                final_mol.SetIntProp("_xyz_skip_checks", 1)
                                break
            ...
            if final_mol:
                ...
                context.show_status_message(
                    f"Pasted {len(atoms_data)} atoms from clipboard."
                )
                ...
                try:
                    if hasattr(mw, "ui_manager") and hasattr(
                        ...
                        mw.ui_manager.minimize_2d_panel()
                    elif hasattr(mw, "init_manager") and hasattr(
