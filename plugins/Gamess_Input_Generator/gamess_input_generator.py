...
PLUGIN_DESCRIPTION = "Generates input files for GAMESS (US) quantum chemistry calculations."
...
class GamessSetupDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.run_type.addItems(
            ["ENERGY", "OPTIMIZE", "HESSIAN", "SADPOINT", "GRADIENT"]
        )
        ...
        self.chk_nosym.setToolTip(
            "Important to prevent 'symmetry unique' errors for arbitrary coordinates."
        )
    ...
    def generate_content(self):
        ...
        if ndfunc > 0:
            basis += f" NDFUNC={ndfunc}"
        if npfunc > 0:
            basis += f" NPFUNC={npfunc}"
        basis += " $END"
        lines.append(basis)
        ...
        if self.mol:
            ...
            for i in range(self.mol.GetNumAtoms()):
                ...
                lines.append(
                    f"{symbol: <4} {an: >3} {pos.x: >12.8f} {pos.y: >12.8f} {pos.z: >12.8f}"
                )
    ...
    def save_file(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save GAMESS Input", default_name, "GAMESS Input (*.inp *.src)"
        )
        if path:
            try:
