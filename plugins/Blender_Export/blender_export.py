...
def initialize(context):
    ...
    context.add_export_action(
        "Export to Blender Script...", lambda: export_to_blender(context)
    )
...
def export_to_blender(context):
    ...
    if not mol or mol.GetNumAtoms() == 0:
        QMessageBox.warning(
            mw,
            "No Molecule",
            "No molecule loaded. Please create or load a molecule first.",
        )
        return
    ...
    try:
        if (
            ...
            and hasattr(mw.init_manager, "current_file_path")
            and mw.init_manager.current_file_path
        ):
    ...
    default_path = (
        os.path.join(default_dir, default_name) if default_dir else default_name
    )
    ...
    try:
        ...
        PLUGIN_CONTEXT.show_status_message(
            f"Blender script exported to {file_path}", 3000
        )
        ...
        QMessageBox.critical(
            mw, "Export Failed", f"Failed to export Blender script:\n{str(e)}"
        )
        PLUGIN_CONTEXT.show_status_message(f"Export failed: {e}", 3000)
...
def generate_blender_script(mol, mw):
    ...
    settings = (
        getattr(mw.init_manager, "settings", {}) if hasattr(mw, "init_manager") else {}
    )
    ...
    color_map = (
        getattr(mw.view_3d_manager, "_3d_color_map", {})
        if hasattr(mw, "view_3d_manager")
        else {}
    )
    raw_style = (
        getattr(mw.view_3d_manager, "current_3d_style", "ball_and_stick")
        if hasattr(mw, "view_3d_manager")
        else "ball_and_stick"
    )
    ...
    if (
        ...
        and hasattr(mw.view_3d_manager, "glyph_source")
        and mw.view_3d_manager.glyph_source is not None
    ):
        try:
    ...
    if current_style == "wireframe":
        ...
        triple_radius_factor = settings.get(
    ...
    if atom_positions:
        ...
        for p in atom_positions:
            dist = math.sqrt(
                (p[0] - center_x) ** 2 + (p[1] - center_y) ** 2 + (p[2] - center_z) ** 2
            )
        ...
        for bond in mol_to_draw.GetBonds():
            ...
            if bond_type == Chem.rdchem.BondType.DOUBLE:
                ...
                if np.allclose(np.abs(np.dot(v1, v_arb)), 1.0):
                    v_arb = np.array([0.0, 1.0, 0.0])
                off_dir = np.cross(v1, v_arb)
                off_dir /= np.linalg.norm(off_dir)

            if (
                bond_type == Chem.rdchem.BondType.SINGLE
                or bond_type == Chem.rdchem.BondType.AROMATIC
            ):
                if f"bond_{bond_counter}" in color_map:
                    # Uniform color
                    color = tuple(
                        float(c) / 255.0 for c in color_map[f"bond_{bond_counter}"]
                    )
                    bonds_data.append(
                        {
                            "pos1": tuple(float(c) for c in p1),
                            "pos2": tuple(float(c) for c in p2),
                    ...
                    c_start = tuple(
                        float(c) / 255.0
                        for c in color_map[f"bond_{bond_counter}_start"]
                    )
                    c_end = tuple(
                        float(c) / 255.0 for c in color_map[f"bond_{bond_counter}_end"]
                    )
                    bonds_data.append(
                        {
                            "pos1": tuple(float(c) for c in p1),
                            "pos2": tuple(float(c) for c in mid),
                            "radius": float(bond_radius),
                            "color": c_start,
                        }
                    )
                    bonds_data.append(
                        {
                            "pos1": tuple(float(c) for c in mid),
                            "pos2": tuple(float(c) for c in p2),
                            "radius": float(bond_radius),
                            "color": c_end,
                        }
                ...
                if bond_type == Chem.rdchem.BondType.DOUBLE:
                    ...
                    s1_start, s1_end = (
                        p1 + off_dir * (s_double / 2),
                        p2 + off_dir * (s_double / 2),
                    )
                    s2_start, s2_end = (
                        p1 - off_dir * (s_double / 2),
                        p2 - off_dir * (s_double / 2),
                    ...
                    if f"bond_{bond_counter}_1" in color_map:
                        color = tuple(
                            float(c) / 255.0
                            for c in color_map[f"bond_{bond_counter}_1"]
                        )
                        bonds_data.append(
                            {
                                "pos1": tuple(float(c) for c in s1_start),
                                "pos2": tuple(float(c) for c in s1_end),
                                "radius": r,
                                "color": color,
                            }
                        )
                        ...
                        c_start = tuple(
                            float(c) / 255.0
                            for c in color_map.get(
                                f"bond_{bond_counter}_1_start", (153, 153, 153)
                            )
                        )
                        c_end = tuple(
                            float(c) / 255.0
                            for c in color_map.get(
                                f"bond_{bond_counter}_1_end", (153, 153, 153)
                            )
                        )
                        bonds_data.append(
                            {
                    ...
                    if f"bond_{bond_counter}_2" in color_map:
                        color = tuple(
                            float(c) / 255.0
                            for c in color_map[f"bond_{bond_counter}_2"]
                        )
                        bonds_data.append(
                            {
                                "pos1": tuple(float(c) for c in s2_start),
                                "pos2": tuple(float(c) for c in s2_end),
                                "radius": r,
                                "color": color,
                            }
                        )
                        ...
                        c_start = tuple(
                            float(c) / 255.0
                            for c in color_map.get(
                                f"bond_{bond_counter}_2_start", (153, 153, 153)
                            )
                        )
                        c_end = tuple(
                            float(c) / 255.0
                            for c in color_map.get(
                                f"bond_{bond_counter}_2_end", (153, 153, 153)
                            )
                        )
                        bonds_data.append(
                            {
                    ...
                    for i, sign in enumerate([1, -1], 2):
                        ...
                        if f"bond_{bond_counter}{suffix}" in color_map:
                            color = tuple(
                                float(c) / 255.0
                                for c in color_map[f"bond_{bond_counter}{suffix}"]
                            )
                            bonds_data.append(
                                {
                                    "pos1": tuple(float(c) for c in s_start),
                                    "pos2": tuple(float(c) for c in s_end),
                                    "radius": r,
                                    "color": color,
                                }
                            )
                            ...
                            c_end = tuple(
                                float(c) / 255.0
                                for c in color_map.get(
                                    f"bond_{bond_counter}{suffix}_end", (153, 153, 153)
                                )
                            )
                            bonds_data.append(
                                {
    ...
    script_lines.append(
        "CLEAR_SCENE = True  # Set to True to clear the entire scene before creating the molecule"
    )
    script_lines.append("")
    ...
    script_lines.append(
        '    """Create a sun light at the given location pointing at the scene center"""'
    )
    ...
    script_lines.append(
        "    light_object.rotation_euler = dir_vec.to_track_quat('-Z', 'Y').to_euler()"
    )
    ...
    script_lines.append(
        "    # For newer Blender versions, ensure auto-smooth is off for spheres"
    )
    script_lines.append("    # but for atoms we generally want pure smooth.")
    script_lines.append(
        "    # If the user says 'not smooth', it might be because of low resolution or auto-smooth limits."
    )
    ...
    script_lines.append(
        "def create_camera(location, look_at, sky, angle=45.0, orthographic=False, parallel_scale=5.0):"
    )
    script_lines.append(
        '    """Create and position the camera to match MoleditPy view exactly"""'
    )
    ...
    script_lines.append("    ")
    script_lines.append(
        "    # In Blender, local Z+ is BACKWARDS. So local Z- is forward."
    )
    ...
    script_lines.append(
        'def create_bond(pos1, pos2, radius=0.1, color=(0.5, 0.5, 0.5), name="Bond"):'
    )
    ...
    script_lines.append(
        "    # Using a cache for materials to avoid creating thousands of them"
    )
    ...
    script_lines.append(
        f'            mat_name = f"{{prefix}}_{{color_key[0]}}_{{color_key[1]}}_{{color_key[2]}}"'
    )
    script_lines.append(
        "            material_cache[mat_key] = create_material(mat_name, color_tuple)"
    )
    ...
    if fwd_norm > 1e-6:
        ...
        script_lines.append(
            f'    create_light(({lp_key[0]:.3f}, {lp_key[1]:.3f}, {lp_key[2]:.3f}), energy=2.5, name="Key_Light")'
        )

        # Rim Light (Back)
        lp_rim = la - fwd_u * (fwd_norm * 0.5) + (sky_u - right_u) * (fwd_norm * 0.4)
        script_lines.append(
            f"    create_rim_light(({lp_rim[0]:.3f}, {lp_rim[1]:.3f}, {lp_rim[2]:.3f}), energy={float(100 * max_dist**2):.1f})"
        )
    else:
        ...
        if current_style == "stick" and atom["degree"] == 1:
            ...
            if bt in [Chem.BondType.DOUBLE, Chem.BondType.TRIPLE]:
                ...
                if bt == Chem.BondType.DOUBLE:
                    ...
                    v_arb = np.array([0, 0, 1])
                    if np.allclose(np.abs(np.dot(bu, v_arb)), 1.0):
                        v_arb = np.array([0, 1, 0])
                ...
                for j, off in enumerate(offsets):
                    ...
                    script_lines.append(
                        f'    mat = get_cached_material({c_val}, "AtomMat")'
                    )
                    script_lines.append(
                        f"    if obj.data.materials: obj.data.materials[0] = mat"
                    )
    ...
    script_lines.append(
        "        mol_collection = bpy.data.collections.new(collection_name)"
    )
    script_lines.append(
    ...
    script_lines.append("    else:")
    script_lines.append(
        "        mol_collection = bpy.data.collections[collection_name]"
    )
    script_lines.append("    ")
    ...
    script_lines.append(
        "            # Skip view_all as it overrides the specific camera placement"
    )
    script_lines.append("            pass")
    script_lines.append("            ")
    ...
    if "camera_data" in locals():
        ...
        script_lines.append(
            f"        location=({camera_data['location'][0]:.6f}, {camera_data['location'][1]:.6f}, {camera_data['location'][2]:.6f}),"
        )
        script_lines.append(
            f"        look_at=({camera_data['look_at'][0]:.6f}, {camera_data['look_at'][1]:.6f}, {camera_data['look_at'][2]:.6f}),"
        )
        ...
        script_lines.append(
            f"        parallel_scale={camera_data.get('parallel_scale', 5.0):.6f}"
        )
        script_lines.append(f"    )")
        script_lines.append("    ")
    ...
    script_lines.append(
        f'    print("Molecular structure created: {len(atoms_data)} atoms, {len(bonds_data)} bonds")'
    )
    script_lines.append("")
