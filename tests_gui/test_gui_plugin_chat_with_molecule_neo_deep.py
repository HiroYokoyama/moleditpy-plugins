"""
Deep GUI coverage for the "Chat with Molecule Neo" (Gemini) plugin.
...
with mock_chemistry_imports():
    _mod = load_plugin_for_gui(GEMINI_PATH)
...
def _fake_genai_with_model(model_name):
    """A fake `genai` module whose Client().models.list() yields one usable model."""
    m1 = MagicMock()
    m1.name = model_name
...
def _sync_workers(monkeypatch):
    ...
    monkeypatch.setattr(_mod.GenAIWorker, "start", lambda self: self.run())
...
class TestInitWorker:
    def test_success_filters_generate_content_models(self, qapp):
        m1 = MagicMock(name="m1", supported_actions=["generateContent"])
        m1.name = "models/gemini-flash-latest"
        m2 = MagicMock(name="m2", supported_actions=["embedContent"])
        ...
        orig = _mod.genai
        _mod.genai = fake_genai
        try:
            worker.run()
        finally:
            _mod.genai = orig

        assert seen == [(["models/gemini-flash-latest"], "")]
    ...
    def test_legacy_supported_generation_methods_field(self, qapp):
        m1 = MagicMock(spec=["name", "supported_generation_methods"])
        m1.name = "models/legacy"
...
class TestGenAIWorker:
    ...
    def test_success_streams_chunks_and_emits_response(self, qapp):
        stream = [self._chunk("Hello "), self._chunk("world")]
        session = MagicMock()
        session.send_message_stream.return_value = stream
        worker = _mod.GenAIWorker(session, "hi")
        chunks = []
        finals = []
        ...
        assert finals == [stream]

    def test_api_error_emits_error_occurred(self, qapp):
        session = MagicMock()
        session.send_message_stream.side_effect = RuntimeError("api down")
        worker = _mod.GenAIWorker(session, "hi")
        errors = []
    ...
    def test_interrupted_emits_interrupted_error(self, qapp):
        stream = [self._chunk("Hello ")]
        session = MagicMock()
        session.send_message_stream.return_value = stream
        worker = _mod.GenAIWorker(session, "hi")
    ...
    def test_stop_sets_interrupted_flag(self, qapp):
        worker = _mod.GenAIWorker(MagicMock(), "hi")
        worker.stop()
...
class TestInitializeSession:
    ...
    def test_no_genai_disables_input(self, qapp, monkeypatch):
        w = self._fresh(qapp, monkeypatch)
        monkeypatch.setattr(_mod, "HAS_GENAI", False)
    ...
    def test_missing_api_key_warns(self, qapp, monkeypatch):
        w = self._fresh(qapp, monkeypatch)
        monkeypatch.setattr(_mod, "HAS_GENAI", True)
        monkeypatch.setattr(_mod.QMessageBox, "warning", MagicMock())
        w.settings = {}
        w.initialize_session()
        assert w.chat_session is None or not hasattr(w, "chat_session") or True
        w.destroy()

    def test_success_with_smiles_sets_pending_context(self, qapp, monkeypatch):
        w = self._fresh(qapp, monkeypatch)
        monkeypatch.setattr(_mod, "HAS_GENAI", True)
        w.settings = {"api_key": "KEY", "model": "models/gemini-flash-latest"}
        fake_genai, fake_client = _fake_genai_with_model("models/gemini-flash-latest")
        monkeypatch.setattr(_mod, "genai", fake_genai)
        ...
        w.initialize_session()
        assert w.chat_session is fake_client.chats.create.return_value
    ...
    def test_no_smiles_sets_context_label(self, qapp, monkeypatch):
        ...
        monkeypatch.setattr(_mod, "HAS_GENAI", True)
        w.settings = {"api_key": "KEY", "model": "models/gemini-flash-latest"}
        fake_genai, _ = _fake_genai_with_model("models/gemini-flash-latest")
        monkeypatch.setattr(_mod, "genai", fake_genai)
    ...
    def test_chat_creation_failure_leaves_session_none(self, qapp, monkeypatch):
        w = self._fresh(qapp, monkeypatch)
        monkeypatch.setattr(_mod, "HAS_GENAI", True)
        w.settings = {"api_key": "KEY", "model": "models/gemini-flash-latest"}
        fake_genai, fake_client = _fake_genai_with_model("models/gemini-flash-latest")
        fake_client.chats.create.side_effect = RuntimeError("boom")
        monkeypatch.setattr(_mod, "genai", fake_genai)
        monkeypatch.setattr(w, "get_current_molecule_smiles", lambda: (None, None))
        w.initialize_session()
        assert w.chat_session is None
        w.destroy()
...
class TestOnInitFinished:
    ...
    def test_saved_model_selected(self, win, monkeypatch):
        ...
        win.settings = {"model": "models/foo"}
        win.on_init_finished(["models/foo", "models/bar"], "")
        assert win.combo_model.currentText() == "models/foo"

    def test_unsaved_model_falls_back_to_flash(self, win, monkeypatch):
        fake_genai = MagicMock()
        monkeypatch.setattr(_mod, "genai", fake_genai)
        monkeypatch.setattr(win, "get_current_molecule_smiles", lambda: (None, None))
        win.settings = {"model": "models/nonexistent"}
        win.on_init_finished(
            ["models/gemini-flash-latest", "models/other"], ""
        )
        assert win.combo_model.currentText() == "models/gemini-flash-latest"

    def test_no_gemini_models_uses_first(self, win, monkeypatch):
        fake_genai = MagicMock()
        monkeypatch.setattr(_mod, "genai", fake_genai)
        monkeypatch.setattr(win, "get_current_molecule_smiles", lambda: (None, None))
        win.settings = {"model": "models/nonexistent"}
        win.on_init_finished(["models/other-x"], "")
        assert win.combo_model.currentText() == "models/other-x"
...
class TestToolActionFlow:
    ...
    def test_propose_single_tool_all_descriptions(self, win, tool_name, params):
        ...
        assert "Gemini suggests" in win.lbl_tool_info.text()
...
class TestSendMessageFlow:
    ...
    def test_no_session_warns(self, qapp, monkeypatch):
        w = self._ready_window(monkeypatch)
        w.chat_session = None
    ...
    def test_success_response_no_tool(self, qapp, monkeypatch):
        w = self._ready_window(monkeypatch)
        chunk = MagicMock()
        chunk.text = "Sure, here's the answer."
        w.chat_session.send_message_stream.return_value = [chunk]
    ...
    def test_error_response_resets_ui(self, qapp, monkeypatch):
        w = self._ready_window(monkeypatch)
        w.chat_session.send_message_stream.side_effect = RuntimeError("network down")
    ...
    def test_stop_generation_resets_ui(self, qapp, monkeypatch):
        ...
        w.stop_generation()
        w.worker.stop.assert_called_once()
        assert w.txt_input.isEnabled()
        w.destroy()
...
class TestResponseCallbacks:
    ...
    def test_log_usage_with_usage_metadata(self, win, monkeypatch):
        logged = []
        monkeypatch.setattr(_mod, "append_log", lambda *a: logged.append(a))
        win.chat_session = MagicMock(history=[1, 2, 3])
        response = MagicMock(usage_metadata="META")
        win.log_usage(response)
        assert ("usage", "META") in logged

    def test_log_usage_triggers_prune_over_max_history(self, win, monkeypatch):
        win.chat_session = MagicMock(history=list(range(_mod.MAX_HISTORY + 5)))
        called = []
        monkeypatch.setattr(win, "_prune_history", lambda: called.append(True))
        win.log_usage(MagicMock(usage_metadata=None))
        assert called == [True]

    def test_prune_history_recreates_session(self, win):
        win.chat_session = MagicMock(history=list(range(30)))
        win.settings = {"model": "gemini-flash-latest", "api_key": "K"}
        win.client = MagicMock()
        win._prune_history()
        win.client.chats.create.assert_called_once()

    def test_prune_history_failure_logged_not_raised(self, win, monkeypatch):
        win.chat_session = MagicMock(history=list(range(30)))
        win.settings = {"model": "m"}
        win.client = MagicMock()
        win.client.chats.create.side_effect = RuntimeError("fail")
        logged = []
        monkeypatch.setattr(_mod, "append_log", lambda *a: logged.append(a))
...
class TestDemoModeSendMessage:
    ...
    def test_all_demo_steps_walk_without_raising(self, qapp, monkeypatch):
        w = self._demo_window(monkeypatch)
        w.demo_step = 0
        for _ in range(14):
            w.txt_input.setPlainText("tell me more")
            w.send_message()
        w.destroy()

    def test_br_shortcut_with_selection(self, qapp, monkeypatch):
        w = self._demo_window(monkeypatch)
        monkeypatch.setattr(w, "get_selected_atom_indices", lambda: ["1", "2"])
        w.txt_input.setPlainText("br please")
        w.send_message()  # must not raise
        w.destroy()

    def test_br_shortcut_no_selection_aborts(self, qapp, monkeypatch):
        w = self._demo_window(monkeypatch)
        monkeypatch.setattr(w, "get_selected_atom_indices", lambda: [])
        w.txt_input.setPlainText("br please")
        w.send_message()
        assert "No atoms selected" in w.chat_history_log[-1]["text"]
        w.destroy()

    def test_load_shortcut(self, qapp, monkeypatch):
        w = self._demo_window(monkeypatch)
        w.txt_input.setPlainText("load p-xylene")
        w.send_message()  # must not raise
        w.destroy()
