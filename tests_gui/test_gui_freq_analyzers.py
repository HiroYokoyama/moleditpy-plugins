"""
Headless GUI tests for ORCA and Gaussian frequency-analyzer dialogs.

Both plugins expose:
  - A main analyser widget (QWidget subclass) that is safe to construct with
    no main-window / no loaded file — all data is populated only when the user
    opens a file, so all data lists start empty and parsers start at None.
  - A standalone SpectrumDialog (QDialog) that accepts empty freq/intensity
    lists; numpy is mocked so np.array([]) returns a MagicMock whose __len__
    returns 0, letting recalc_curve() exit early without crashing.
  - Pure-Python parser objects (OrcaParser, FCHKParser) that need no Qt.

Run:
    QT_QPA_PLATFORM=offscreen pytest tests_gui/test_gui_freq_analyzers.py -v
"""
