"""
Deep GUI coverage for the "Chat with Molecule Neo" (Local) plugin.
...
Worker classes (InitWorker/OpenAIWorker) are QThread subclasses; instead of
...
with mock_chemistry_imports():
    _mod = load_plugin_for_gui(LOCAL_PATH)
_mod.HAS_MARKDOWN = False  # avoid mocked markdown.markdown() poisoning render_content
...
def _make_mw(atoms=None, bonds=None, selected=frozenset()):
    ...
    # Local reads scene via BOTH self.main_window.scene and
    # self.init_manager.scene (cached alias) -- keep them the same object.
    mw.init_manager.scene = mw.scene
    return mw
...
def _make_real_main_window():
    ...
    (and super().__init__(main_window) requires a real QWidget or None), so
    it needs a bare QDialog with MagicMock manager attrs attached -- a
    MagicMock() itself is rejected by Qt as an invalid parent type.
    """
...
def _build_window(monkeypatch, ctx=None, stub_init=True):
    ...
    if ctx is None:
        ctx = MagicMock()
        ctx.get_main_window.return_value = _make_real_main_window()
...
def _fake_openai_with_models(*model_ids):
    """A fake `openai` module whose OpenAI().models.list() yields models with
    the given ids (Local returns ALL models, unfiltered, unlike ChatGPT)."""
    items = []
    for mid in model_ids:
        ...
        items.append(m)
    client = MagicMock()
    client.models.list.return_value = items
...
class TestInitWorker:
    def test_success_returns_all_models_sorted_unfiltered(self, qapp):
        fake_openai, client = _fake_openai_with_models("llama-3", "custom-model-a")
        worker = _mod.InitWorker("key123", "http://localhost:1234/v1")
        assert worker.base_url == "http://localhost:1234/v1"
        seen = []
        ...
        try:
            ...
            _mod.openai = orig
        assert seen == [(["custom-model-a", "llama-3"], "")]

    def test_error_emits_empty_list_and_message(self, qapp):
        fake_openai = MagicMock()
        fake_openai.OpenAI.side_effect = RuntimeError("connection refused")
        worker = _mod.InitWorker("key123", "http://localhost:1234/v1")
        seen = []
        worker.finished.connect(lambda models, err: seen.append((models, err)))
        orig = _mod.openai
        ...
        try:
            ...
            _mod.openai = orig
        assert seen == [([], "connection refused")]
...
class TestOpenAIWorker:
    ...
    def test_interrupted_emits_interrupted_error(self, qapp):
        # Local's stream close-on-interrupt calls stream.response.close(); a
        # plain list has no such attribute, which is silently swallowed.
...
class TestGetCurrentMoleculeSmiles:
    ...
    def test_no_main_window(self, win):
        ...
        win.state_manager = None
        win.init_manager = None
        win.edit_actions_manager = None
        smiles, err = win.get_current_molecule_smiles()
        assert smiles is None and "Internal Error" in err

    def test_empty_scene_no_fallback(self, win):
        win.main_window = _make_mw(atoms={})
        win.state_manager = win.main_window.state_manager
        win.init_manager = win.main_window.init_manager
        win.edit_actions_manager = win.main_window.edit_actions_manager
...
class TestGetSelectedAtomIndices:
    def test_no_data_attr_returns_empty(self, win):
        ...
        win.state_manager = MagicMock(spec=[])
        win.init_manager = MagicMock(spec=[])
        win.edit_actions_manager = MagicMock(spec=[])
...
class TestInitializeSession:
    ...
    def test_missing_api_base_shows_message(self, qapp, monkeypatch):
        w = self._fresh(qapp, monkeypatch)
        monkeypatch.setattr(_mod, "HAS_OPENAI", True)
        w.settings = {}
        w.initialize_session()
        assert "Local API Base URL" in w.chat_history_log[-1]["text"]
    ...
    def test_success_with_smiles_sets_pending_context(self, qapp, monkeypatch):
        ...
        fake_openai, fake_client = _fake_openai_with_models("llama-3")
    ...
    def test_no_smiles_sets_context_label(self, qapp, monkeypatch):
        ...
        fake_openai, _ = _fake_openai_with_models("llama-3")
        monkeypatch.setattr(_mod, "openai", fake_openai)
...
class TestOnInitFinished:
    ...
    def test_saved_model_selected(self, win, monkeypatch):
        ...
        win.settings = {"model": "llama-3"}
        win.on_init_finished(["llama-3", "other-model"], "")
        assert win.combo_model.currentText() == "llama-3"

    def test_unsaved_model_falls_back_to_first(self, win, monkeypatch):
        fake_openai = MagicMock()
        monkeypatch.setattr(_mod, "openai", fake_openai)
        monkeypatch.setattr(win, "get_current_molecule_smiles", lambda: (None, None))
...
class TestFetchModels:
    def test_missing_api_key_uses_dummy_key(self, win, monkeypatch):
        # Local doesn't warn on an empty key -- it substitutes "lm-studio"
        # (many local servers don't require one).
        win.txt_api_key.setText("")
        win.txt_api_base.setText("http://localhost:1234/v1")
        fake_openai, client = _fake_openai_with_models("llama-3")
        monkeypatch.setattr(_mod, "openai", fake_openai)
        win.fetch_models()
    ...
    def test_success_populates_combo(self, win, monkeypatch):
        ...
        win.txt_api_base.setText("http://localhost:1234/v1")
        fake_openai, _client = _fake_openai_with_models("llama-3")
        monkeypatch.setattr(_mod, "openai", fake_openai)
        win.fetch_models()
        assert win.combo_model.count() >= 1
...
class TestToolActionFlow:
    ...
    def test_propose_single_tool_all_descriptions(self, win, tool_name, params):
        ...
        assert " suggests:" in win.lbl_tool_info.text()
...
class TestSendMessageFlow:
    ...
    def test_success_response_no_tool(self, qapp, monkeypatch):
        ...
        w.send_message()
        assert any("answer" in e["text"] for e in w.chat_history_log)
        assert w.txt_input.isEnabled()
        w.destroy()
...
class TestResponseCallbacks:
    ...
    def test_log_usage_falls_back_to_usage_metadata(self, win, monkeypatch):
        logged = []
        monkeypatch.setattr(_mod, "append_log", lambda *a: logged.append(a))
        win.chat_history_state = [1, 2, 3]
        response = MagicMock(spec=["usage_metadata"], usage_metadata="META")
        win.log_usage(response)
